...
CREATE TABLE `corretoras` (
    ...
    `segmento` ENUM('SIM', 'NAO') NOT NULL,
    `centralAtendimento` VARCHAR(191) NULL,
    `site` VARCHAR(191) NULL,
    `email` VARCHAR(191) NULL,
    `status` ENUM('NOVO', 'EM_TESTES', 'AGUARDANDO_PAGAMENTO', 'ATIVO') NOT NULL,
    `diasParaEntrarEmContato` INTEGER NULL,
    `emailDiario` ENUM('SIM', 'NAO') NOT NULL,
    `faseImportacaoLeads` ENUM('AGUARDANDO_CONTATO', 'CONTACTADO', 'FECHADO_CONVERTIDO', 'FECHADO_NAO_CONVERTIDO') NOT NULL,
    `origemImportacaoLeads` INTEGER NOT NULL,
...
CREATE TABLE `clientes` (
    ...
    UNIQUE INDEX `clientes_cnpjCpf_key`(`cnpjCpf`),
    PRIMARY KEY (`id`)
) DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
...
-- CreateTable
CREATE TABLE `unidadenegocios` (
    `id` INTEGER NOT NULL AUTO_INCREMENT,
    `nome` VARCHAR(191) NOT NULL,
    `responsavel` VARCHAR(191) NULL,
    `susep` VARCHAR(191) NULL,
    `email` VARCHAR(191) NULL,
    ...
    UNIQUE INDEX `unidadenegocios_email_key`(`email`),
    PRIMARY KEY (`id`)
) DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
...
CREATE TABLE `produtos` (
    ...
    `descricao` TEXT NULL,
    `ativo` BOOLEAN NOT NULL DEFAULT true,
    `exibirNoOrcamento` BOOLEAN NOT NULL DEFAULT false,
    `comissaoSobreAdicional` BOOLEAN NOT NULL DEFAULT false,
    `iof` DECIMAL(8, 2) NULL,
    `seguroPorAssinatura` BOOLEAN NOT NULL DEFAULT false,
    `valorPrimeira` DECIMAL(10, 2) NULL,
    `valorDemais` DECIMAL(10, 2) NULL,
    `questionarioDeVenda` TEXT NULL,
    `ramoId` INTEGER NOT NULL,
    `seguradoraId` INTEGER NOT NULL,
    `createdAt` DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3),
    `updatedAt` DATETIME(3) NOT NULL,
...
-- AddForeignKey
ALTER TABLE `produtos` ADD CONSTRAINT `produtos_ramoId_fkey` FOREIGN KEY (`ramoId`) REFERENCES `ramos`(`id`) ON DELETE RESTRICT ON UPDATE CASCADE;

-- AddForeignKey
ALTER TABLE `produtos` ADD CONSTRAINT `produtos_seguradoraId_fkey` FOREIGN KEY (`seguradoraId`) REFERENCES `seguradoras`(`id`) ON DELETE RESTRICT ON UPDATE CASCADE;
